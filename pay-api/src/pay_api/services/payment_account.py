...
class PaymentAccount():  # pylint: disable=too-many-instance-attributes, too-many-public-methods
    ...
    def _save_account(cls, account_request: Dict[str, any], payment_account: PaymentAccountModel):
        ...
        if not payment_method or payment_method == PaymentMethod.PAD.value:
            new_payment_method, new_activation_date = \
                PaymentAccount._get_payment_based_on_pad_activation(payment_account)
            payment_account.pad_activation_date = new_activation_date
            payment_account.payment_method = new_payment_method
